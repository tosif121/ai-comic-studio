...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Call ElevenLabs API with user's key
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { success: false, error: `ElevenLabs API failed`, details: errorText },
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to generate narration', details: error.message },
      { status: 500 }
    );
  }
}
